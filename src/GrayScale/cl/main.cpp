...
/* cl variables */
std::string sourceName("GrayScale.cl");
...
void help(const std::string & filename)
{
    std::cout
        ...
        << std::endl
        << " * Examples *" << std::endl
        << filename << " [OPTS...] -v -w 256 test_data.ppm" << std::endl
...
int main(int argc, char **argv)
{
    ...
    verbose && std::cerr << "GRAYSCALE, OpenCL Implementation with " << ((deviceType==CL_DEVICE_TYPE_GPU)?("GPU"):("CPU"))
                         << std::endl << std::endl
    ...
    GrayScaleCL GrayScaleCL(inp, platformId, deviceType, kernelVer);

    GrayScaleCL.init();
    for (int i = 0; i < N_ITER; i++) {
        GrayScaleCL.prep_memory();
        GrayScaleCL.execute();
        std::cerr << "Iteration " << i+1 << "/" << N_ITER << ": DONE." << std::endl;

        GrayScaleCL.copyback();
        GrayScaleCL.compare_to_cpu();

        if (outputFilename.size() == 0)
            outputFilename = "GrayScale_ocl_" + realName + "_out" + prefix;     /* Produce [filename]_out */
        verbose
            && std::cerr << "Producing output to: " << outputFilename << std::endl;

        outParam output;
        output.outputFilename = outputFilename;
        GrayScaleCL.output(reinterpret_cast < void *>(&output));

        GrayScaleCL.clean_mem();
    }
    GrayScaleCL.finish();
