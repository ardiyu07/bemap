...
static const float filter[121] =        // arbitrary circularly symmetric filter -> AT * A
{ 
                            ...
                            int h, int w) 
{
    ...
    for (i = 0; i < height; i++)
    {
        for (j = 0; j < width; j++)
        {
            ...
            rr = MAX(0.0f, MIN(255.0f, rr));
            gg = MAX(0.0f, MIN(255.0f, gg));
            bb = MAX(0.0f, MIN(255.0f, bb));
